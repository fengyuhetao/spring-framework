...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 * 利用classLoader加载指定路径下的资源，不管实在class路径下还是在jar包
	 * 如果传入空或者/，加载所有的jar包
	 * <p>
	...
	protected Set<Resource> doFindAllClassPathResources(String path) throws IOException {
		...
		// 如果传入resourceLoader的时候传入了ClassLoader, 根据classloader加载路径下所有的资源
	...
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		// 找到第一个通配符所在目录，获取通配符前边的字符串
		// classpath*:test/cc*/spring-*.xml        =>          classpath*:test/
		String rootDirPath = determineRootDir(locationPattern);
		String subPattern = locationPattern.substring(rootDirPath.length());

		// rootDir有三个，在build目录中：classes/java/main classes/java/test  resources/test
	...
	/**
	 * 遍历rootDir中的所有文件，根据subPattern进行匹配
	 *
	 * Find all resources in the file system that match the given location pattern
	 * via the Ant-style PathMatcher.
	 *
