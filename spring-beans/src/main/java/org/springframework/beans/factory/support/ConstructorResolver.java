...
class ConstructorResolver {
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		if (explicitArgs != null) {
			...
			// 如果给定的构造函数 A(int, int), 则通过此方法后会把配置文件中的（"1"， "1"）转换为（1，1）
			// 缓存中的值可能是原始值也可能是最终值
			if (argsToResolve != null) {
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			if (candidates.size() > 1) {  // explicitly skip immutable singletonList
				// public构造函数有限参数数量降序，非public构造函数参数数量降序
			...
			if (explicitArgs != null) {
				...
				// getBean() 没有传递参数，则需要解析保存在BeanDefinition构造函数中指定的参数
				...
				if (parameterCount >= minNrOfArgs) {
					...
					if (explicitArgs != null) {
						...
						try {
							...
							// 获取ParameterNameDiscover对象
							// ParameterNameDiscover是用于解析方法和构造函数的参数名称的接口，为参数名称探测器
							ParameterNameDiscoverer pnd = this.beanFactory.getParameterNameDiscoverer();
							if (pnd != null) {
								// 获取指定构造函数的参数名称
								paramNames = pnd.getParameterNames(candidate);
							...
							// 在已经被解析的构造函数参数值的情况下，创建一个参数持有者对象
